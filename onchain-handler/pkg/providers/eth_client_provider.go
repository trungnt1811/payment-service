...
var (
	clientMap = make(map[constants.NetworkType]interfaces.Client)
...
func ProvideEthClient(network constants.NetworkType, rpcUrls []string) (interfaces.Client, error) {
	clientMux.Lock()
	...
	client, err := eth.NewRoundRobinClient(rpcUrls)
	if err != nil {
